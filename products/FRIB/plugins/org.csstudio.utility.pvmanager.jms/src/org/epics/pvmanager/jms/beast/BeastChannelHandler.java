...
public class BeastChannelHandler extends
	...
	private void createInitBeastMapMessage() throws Exception {
		...
		final AlarmConfiguration config = new AlarmConfiguration(Preferences.getRDB_Url(),Preferences.getRDB_User(),
			 ...
			 protected IStatus run(IProgressMonitor monitor) {
				 monitor.beginTask(Messages.AlarmClientModel_ReadingConfiguration, IProgressMonitor.UNKNOWN);
					try {
						...
						map.setString(JMSAlarmMessage.VALUE, pv.getValue()==null?"":pv.getValue());
						map.setString(JMSLogMessage.SEVERITY, pv.getSeverity().name());
						map.setString(JMSAlarmMessage.STATUS, pv.getCurrentMessage());
						map.setString(JMSAlarmMessage.EVENTTIME, new SimpleDateFormat(
								JMSLogMessage.DATE_FORMAT, Locale.ENGLISH).format(pv.getTimestamp()==null?new Date():pv.getTimestamp().toDate()));
						processMessage(map);
					} catch (Exception e) {
						e.printStackTrace();
